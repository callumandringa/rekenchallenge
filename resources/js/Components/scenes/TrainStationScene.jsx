...
const TrainStationScene = ({ onSolve, vragen, solvedObjects }) => {
  ...
  const addTimeNotationHint = (vraag) => {
    if (vraag && (vraag.vraag.toLowerCase().includes("tijd") || vraag.vraag.toLowerCase().includes("uur"))) {
      return <p className="text-sm text-blue-600 italic mb-2">Noteer tijden met een dubbele punt (bijv. 8:30)</p>
    }
    return null
  }
  ...
  return (
    <div
      ...
      style={{ backgroundImage: "url('/images/TreinScene.jpg')" }}
    >
      <h2 className="text-4xl text-center text-white p-6 font-bold drop-shadow-lg">Treinstation</h2>

      {/* Improved object buttons */}
      <button
        className={`absolute left-40 top-40 bg-blue-600 hover:bg-blue-700 text-white p-4 rounded-lg shadow-lg transform transition-all duration-300 hover:scale-105 focus:outline-none focus:ring-4 focus:ring-blue-300 ${
